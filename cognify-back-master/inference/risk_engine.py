...
class RiskEngine:
    ...
    def _load_calibration(self, path: str):
        ...
        try:
            ...
            self.logistic_coefs = np.array([
                params['calibration_model']['coefficients'][0],
                params['calibration_model']['coefficients'][1],
                params['calibration_model']['coefficients'][2],
                params['calibration_model']['coefficients'][3],
                params['calibration_model']['coefficients'][4]
            ])
    ...
    def compute_decline_probability(
        ...
        Input: [NeuroRisk, CardioRisk, FrailtyRisk, gaitSpeedMs_accel, hrvSdnnMs_accel]
    ...
    def assess_risk(
        ...
        try:
            ...
            # Get latest drifts for explainability and features
            feature_drifts = self.dispatcher.extract_latest_feature_drifts(
                processed_df,
                drift_type='long'
            )
            ...
            # Calculate accelerations for the 5-feature vector
            gait_accel = feature_drifts_short.get('gaitSpeedMs', 0.0) - feature_drifts.get('gaitSpeedMs', 0.0)
            hrv_accel = feature_drifts_short.get('hrvSdnnMs', 0.0) - feature_drifts.get('hrvSdnnMs', 0.0)
            
            # Get domain-level drift breakdown for features
            domain_drifts = self.dispatcher.compute_all_domain_drifts(feature_drifts)
            
            # Build the 5-feature array for decline probability using drifts, matched to training
            X = np.array([
                domain_drifts.get('Neuro', 0.0),
                domain_drifts.get('Cardio', 0.0),
                domain_drifts.get('Frailty', 0.0),
                gait_accel,
                hrv_accel
            ])
            ...
            decline_probability = self.compute_decline_probability(X)
